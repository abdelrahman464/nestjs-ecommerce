...
- Inventory ledger — `POST/GET /api/v1/inventory/...` (append-only `inventory_movements` + levels + cached `product_variants.stock`)
- Warehouses — `CRUD /api/v1/warehouses` + per-location levels / transfers
...
3. **Usage:**
   - **Initiate Google Login:** `GET /api/v1/auth/google`
   - **Callback:** `GET /api/v1/auth/google/callback` (handled automatically)
   - Users will be redirected to Google for authentication
   - After successful authentication, users are logged in and receive JWT tokens

**Features:**

- Automatic user creation if email doesn't exist
- Account linking if email already exists (links Google account to existing user)
